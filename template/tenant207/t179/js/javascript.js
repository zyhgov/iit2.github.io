...
(function ($, undefined) {
	...
	window.touchSlider = function (options) {
		...
		var navCreator = function navCreator(paginationWrap, images) {
		...
		var crossLeft = isTouchWebkit ? function (elem, px, duration) {
			...
			elem.css({
				...
				webkitTransform: function webkitTransform(i) {
		...
		// 更新当前播放进度
		var changePrgs = function changePrgs() {
   ...
   */
		var loadImage = function loadImage(cNode) {
			// 加载当前图片
			var links = cNode.find('a'),
			    img,
			    dataSrc;
		...
		var switching = function () {
			...
			return {
				...
				move: function move(e, previousPageX) {
				...
				moveEnd: function moveEnd(e, pxInMs, directionX, startTime, distX, distY) {
		...
		function changedView(index) {
			...
			pagination.removeClass(options.currentClass).eq(parseInt(index)).addClass(options.currentClass);
	...
	//(window.touchSlider,'window.touchSlider');
...
// 	function getTargetInfo(n) 
// 	{
// 		var mt = today.valueOf();
// 		var uom = new Date(mt-n*24*60*60*1000);
// 		var Y = uom.getFullYear();
// 		var M = pad((uom.getMonth()+1),2);
// 		var D = pad(uom.getDate(),2);
// 		jfjb = host + '/jfjbmap/content/1/' + (Y + "-" + M + "/" + D+"/01/"+Y+M+D+"01_brief.jpg").replace(/\b(\w)\b/g, '0$1');
// 		zggfb = host + '/gfbmap/content/21/' + (Y + "-" + M + "/" + D+"/01/"+Y+M+D+"01_brief.jpg").replace(/\b(\w)\b/g, '0$1');
// 	}

// 	function Fixjfjb(){
// 		$('#jfjbcover').attr({'src':jfjb});
...
// 	}else{
// 		getTargetInfo(0);
// 		$('#jfjbhidden').attr({src:jfjb});		
// 		$('#jfjbhidden').load(function(){
// 			$('#jfjbcover').attr({'src':jfjb});
// 		}).error(function(){
// 			getTargetInfo(1);
// 			$('#jfjbcover').attr({'src':jfjb});
...
// 	var i = 0;
// 	function fillgfb() {
// 		$('#zggfbhidden').attr({src:zggfb});	
// 		$('#zggfbhidden').load(function () {
// 			$('#zggfbcover').attr({ 'src': zggfb });
// 			return;
// 		}).error(function(){
// 			i++;
// 			getTargetInfo(i);
...
//   text = trim(text)
  ...
  var compute = function (innerWidth) {
    ...
    if (swipe == screenName) {
      ...
      var oneUrl = topline.find('[data=one-url]')
      if (oneUrl.length > 0) {
      ...
      fSize = fSize - 3
      title.attr('style', styleStr + 'font-size:' + fSize + 'px;visibility:visible');
      title.find('*').removeAttr('style')
      title.find('*').attr('style', styleStr + 'font-size:' + fSize + 'px;visibility:visible');
    }
  }
...
function TitleSwitch(index) {
	...
	var iitem = links.eq(index);

	for (var i = 0; i <= links.length; i++) {
		links.eq(i).find('a').text(links.eq(i).attr('data-s'));
	}

	iitem.find('a').text(iitem.attr('data-f'));
}
...
// });
$(".focus-main").touchSlider({ autoplay: true });
$(".touchslider-ad").touchSlider({ autoplay: false });
cm_tab();
// loadJbMap();
resTopline();
loadJbMap();
...
// 解决小米手机轮播图失效问题
$('.focus-main[data-scale="0.5625"]').each(function(index, el) {
...
// 动态获取 解放军报/中国国防报。
// https://rmt-zuul.jfjb.com.cn/api-paper/api/newestPaper
function loadJbMap() {
	$.ajax({
		...
		success: function success(res) {
			...
			data.map(function (item) {
				...
				if (item.paperName === '解放军报') {
					$('#APP-NewsPaper img').attr('src', item.paperImg);
					$('#APP-NewsPaper a').attr('href', urla);
					$('#jfjbMenu').attr('href', urla);
				}
				if (item.paperName === '中国国防报') {
					$('#APP-ChPaper img').attr('src', item.paperImg);
					$('#APP-ChPaper a').attr('href', urla);
					$('#gfbMenu').attr('href', urla);
				}
			});
		}
	});
...
+function () {
	var links = $('.if-empty-hidden');
	links.each(function (x, e) {
		$('.if-empty-hidden').each(function (e, x) {
			if (!$(x).attr('href').length) {
				$(x).hide();
			}
		});
	});
