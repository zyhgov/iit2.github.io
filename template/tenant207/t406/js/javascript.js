/*
支持手机的焦点图
TouchSlider 0.95 || Licensed under the MIT license. || http://touchslider.com */
+function($, undefined) {
	window.touchSlider = function(options) {
		...
		if (scroller.css("position") !== "absolute") {
			var viewportHeight = viewport.width()*0.5625;
			viewport.css({
					...
					if (endCoords[0] + scrollerLeft > 0) { // space in left
						...
						if (onFly) {
							...
							if (maximumInViewport === touched &&
								...
								distX + distY > Math.sqrt(Math.pow(viewport.width()*0.5625, 2) + Math.pow(viewportWidth, 2)) * 0.05)
							{
		...
		$(window).resize(function(){
			...
			scroller.css({height: viewportHeight});
		});
...
/*TAB 切换*/
+function youth_tab(){	
	$('.tab-wrap').find('.tab-handle li').hover(
		function(){
			_this = $(this);
			trigger = setTimeout(function(){
				var CMTAB = _this.parents('.tab-wrap');
				var TabHandle = CMTAB.find('.tab-handle li');
				var TabItem = CMTAB.find('div.tab-item');
				var TabIndex = TabHandle.index(_this);			
				TabHandle.stop(true,true).removeClass("active");
				_this.addClass('active');
				TabItem.removeClass('show').eq(TabIndex).addClass('show');
			},200);			
		},function(){
	...
	$('.tab-wrap').find('.tab-handle li a').bind('click', function(event){
...
+function($){
	$(".slider").touchSlider({namespace:"slider"});
}(jQuery);

+function($){
	var qrcode = new QRCode(document.getElementById('qrcode-box'),{width:180,height:180});
	var qrcodebox = $('#qrcode-box');
	var showbox = function(e){
		//调整二维码位置
		//修改前：qrcodebox.css({left:e.offsetLeft+e.offsetWidth+20,top:e.offsetTop-180});
		qrcodebox.css({left:e.offsetLeft+e.offsetWidth,top:e.offsetTop-220});
		qrcodebox.show();
		qrcodebox.focus();
	}
	qrcodebox.focusout(function(){
		qrcodebox.hide();
	});
	$('.qrcode').click(function(){
		qrcode.makeCode($(this).parent().find('a').attr('href'));
		showbox($(this)[0]);
	});
}(jQuery);
