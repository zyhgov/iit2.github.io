...
(function($, undefined) {
	window.touchSlider = function(options) {
        ...
        var loadImage = function(cNode){
            ...
            var _currentNum = parseInt(index%totail || totail ).toString();
			if(_currentNum.length<2){
				_currentNum = '0' + _currentNum;
			}
            cNode.find('.position').html('<sup>'+ _currentNum +'</sup>'+'<sub>'+totail+'</sub>');
		...
		function changedView(index) {			
			...
			var height =  parseInt($(cur).outerHeight())+6;
			var top = parseInt(index)*height - height*1;
			top = top > 8*height+6 ? 8*height+6 : top;
...
;(function () {		  
	...
	var fullHeight = $('.top-stories-reference').height()-60;
	var topstories = $('.top-stories');

	var articles = $('.top-stories .article');
	$.each(articles, function (index, item) {
		// console.log($(item).attr('data-keyword') )
		if ($(item).attr('data-keyword').search('major') < 0) {
			// console.log($(item).attr('data-keyword') )
			// if ($(item).attr('data-keyword').search('major') < 0) {
			...
			$(item).addClass('major').find('img').length > 0 && $(item).addClass('photo');
		
	});


	var targetUrl = $('#topstories a').attr('href');
	topstories.find('.normal .title').append("<a target='_blank' class='link-more' href="+targetUrl+"></a>")
